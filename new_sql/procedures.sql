...
BEGIN
    ...
    ELSE
        ...
        SET 
            ...
            description = dish_des
        WHERE id = dish_code;
    END IF;
END$$

CREATE PROCEDURE `count_dish` (
...
BEGIN
    ...
    FROM 
        dish_group
        JOIN dish_dish_group 
            ON dish_dish_group.dish_group = dish_group.id
        JOIN dish 
            ON dish.id = dish_dish_group.dish     
    WHERE dish.price >= floor_price
    GROUP BY dish_group.name
    HAVING quantity >= 1 
    ORDER BY quantity, dish_group.name;
END$$


CREATE PROCEDURE `count_point` (
    IN `point` INT
)   
BEGIN
