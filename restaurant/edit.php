...
$moTaNhaHang = htmlspecialchars($_POST['moTaNhaHang'] ?? '');
$anhDaiDien = htmlspecialchars($_POST['anhDaiDien'] ?? '');
$path = $_FILES['fileToUpload1']['name'] ?? '';

if (empty($tenDangNhap) || empty($tenNhaHang) || empty($diaChi) || empty($sdt)) {
...
$conn = OpenCon();
$query = "CALL update_restaurant('$tenDangNhap', '$tenNhaHang', '$diaChi', '$sdt', '$target_file', '$moTaNhaHang');";
