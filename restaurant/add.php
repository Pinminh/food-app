...
$tenNhaHang = htmlspecialchars($_POST['tenNhaHang'] ?? '');
$diaChi = htmlspecialchars($_POST['diaChi'] ?? '');
$sdt = htmlspecialchars($_POST['sdt'] ?? '');
$moTaNhaHang = filter_input(INPUT_POST,'moTaNhaHang', FILTER_SANITIZE_SPECIAL_CHARS);

$path = $_FILES['fileToUpload']['name'] ?? null;

if (empty($tenDangNhap) || empty($matKhau) || empty($tenNhaHang) || empty($diaChi) || empty($sdt) || empty($path)) {
...
$conn = OpenCon();
$query = "CALL add_restaurant('$tenDangNhap', '$matKhau', '$tenNhaHang', '$diaChi', '$sdt', '$target_file', '$moTaNhaHang');";

try {
    if ($conn->query($query) === TRUE) {
        echo json_encode(['success' => 'Thêm nhà hàng thành công']);
    } else {
